---
name: web-business-telemetry
description: "Verify Google Search Console and Google Analytics for a deployed Web business, record existing or explicitly authorized property setup, read back authorized aggregate conversion or value events, separate indexing from performance data, schedule day-7 and day-14 observation when data is missing, and return the pipeline to observing. Use for Web 业务 GSC/GA、统计回读、价值事件回读、索引检查、sitemap、telemetry_verified、observing、第 7 天或第 14 天复盘；not for generic GSC SEO analysis, adding tracking without authorization, Google Indexing API submission, fabricated traffic or conversion values, credential capture, growth decisions, or creating properties without authorization."
---
...
- existing aggregate conversion/value events keep raw counts, definitions, period and source
...
3. 分别记录 GSC 与 GA 的 property ID、setup status、setup mode、readback time、data status、时间区间和真实指标。只读取页面/API 明确返回的值；无数据、权限错误和请求失败是不同状态。若现有授权 GA property 已返回转化或价值事件，在 `ga.metrics` 中保留事件定义、聚合原始计数和实际返回值，并把 `unknown`、`zero`、`not_applicable` 分开；第三方支付、广告或 Affiliate 后台数据不得伪装成 GA 数据。
...
6. 初次模式先 `gate --target telemetry_verified` 并转移，再补齐 observation 后 `gate --target observing` 并转移。`hold` 复查或 grow 优化/扩展模式在新 snapshot 和全部前置证据更新后直接 gate/transition 到 `observing`。
7. 最终分开报告 property 配置、索引、搜索性能、已有转化/价值事件、观察周期和 missing evidence；增长及商业结论交给 `$web-business-growth`。
...
- 索引、sitemap 和技术检查证据
- 已有授权埋点中的聚合原始事件，或明确的 unknown/not_applicable
...
- 不新增或修改站点埋点，不保存原始 IP、用户标识或逐用户行为；埋点实现必须另行规划和授权。
...
- 用零填补尚未返回的数据
- 把 missing event、第三方收入或模型估算写成 GA 转化事实
