...
| State | Proof added |
|---|---|
| `candidate_locked` | Human-approved, qualified, platform-disambiguated candidate is immutable |
...
| `local_verified` | Files and hashes match, required checks pass, first batch is human-reviewed |
...
The state file stores a canonical SHA-256 of the lock and a second identity hash over `key + platform_ids`. A Steam and Roblox title with the same normalized name can therefore retain the radar key while remaining distinguishable.
