#!/usr/bin/env python3
"""Deterministic state and evidence gates for the game-site pipeline.
...
SENSITIVE_CLAIM_TYPES = {"redeem_code", "numeric_value", "official_link"}
...
def validate_candidate(candidate: dict[str, Any], locked: bool = True) -> list[str]:
    ...
    if not is_nonempty_string(key) or re.fullmatch(r"game:[a-z0-9][a-z0-9-]*", key) is None:
        errors.append("candidate key must be a non-empty game:<slug> value")
    ...
    platform_ids = candidate.get("platform_ids")
    if not isinstance(platform_ids, list) or not platform_ids:
        errors.append("candidate platform_ids must contain at least one platform identity")
    else:
        seen_platform_ids: set[tuple[str, str]] = set()
        for index, item in enumerate(platform_ids):
            if not isinstance(item, dict):
                errors.append(f"platform_ids[{index}] must be an object")
                continue
            platform = item.get("platform")
            platform_id = item.get("id")
            if not is_nonempty_string(platform) or not is_nonempty_string(platform_id):
                errors.append(f"platform_ids[{index}] requires non-empty platform and id")
                continue
            identity = (platform.casefold(), platform_id.casefold())
            if identity in seen_platform_ids:
                errors.append(f"duplicate platform identity: {platform}:{platform_id}")
            seen_platform_ids.add(identity)

    qualification = candidate.get("qualification")
    if not isinstance(qualification, dict):
        errors.append("candidate qualification must be an object")
    else:
        if qualification.get("trends_status") != "rising":
            errors.append("qualification.trends_status must be rising")
        kd = qualification.get("semrush_kd")
        if not isinstance(kd, (int, float)) or isinstance(kd, bool) or not 0 <= kd < 30:
            errors.append("qualification.semrush_kd must be a number from 0 up to but not including 30")
        volume = qualification.get("semrush_volume")
        if not isinstance(volume, (int, float)) or isinstance(volume, bool) or volume <= 0:
            errors.append("qualification.semrush_volume must be a positive measured value")
        if not is_nonempty_string(qualification.get("semrush_database")):
            errors.append("qualification.semrush_database is required")
        long_tail = qualification.get("long_tail_count")
        if not isinstance(long_tail, int) or isinstance(long_tail, bool) or long_tail < 10:
            errors.append("qualification.long_tail_count must be at least 10")
        if qualification.get("serp_status") not in {"open", "mixed"}:
            errors.append("qualification.serp_status must be open or mixed")
        reliable = qualification.get("reliable_source_count")
        if not isinstance(reliable, int) or isinstance(reliable, bool) or reliable < 2:
            errors.append("qualification.reliable_source_count must be at least 2")
...
def validate_state(state: dict[str, Any], candidate: dict[str, Any]) -> list[str]:
    ...
    if state.get("candidate_identity") != canonical_json_sha256(
        {"key": candidate.get("key"), "platform_ids": candidate.get("platform_ids")}
    ):
        errors.append("pipeline-state.candidate_identity does not match the locked platform identity")
        ...
        for index, claim in enumerate(claims):
            ...
            if claim_type in SENSITIVE_CLAIM_TYPES:
                ...
                if not current_trusted:
                    errors.append(
                        f"sensitive claim {claim_id} requires a current official or trusted source"
                    )
                if claim.get("status") != "verified":
                    errors.append(f"sensitive claim {claim_id}.status must be verified")
...
def validate_launch_report(
    ...
    if require_local:
        ...
        reviewed = [page for page in pages if isinstance(page, dict) and page.get("human_reviewed") is True]
        required_reviews = min(5, len(pages))
        if len(reviewed) < required_reviews:
            errors.append(
                f"human review gate requires {required_reviews} pages; only {len(reviewed)} are recorded"
            )
        for page in pages:
            if not isinstance(page, dict):
                continue
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    candidate_lock = {
        ...
        "platform_ids": candidate_input["platform_ids"],
        "decision": {
            "status": "approved",
    ...
    identity = canonical_json_sha256(
        {"key": candidate_lock["key"], "platform_ids": candidate_lock["platform_ids"]}
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Govern a game-site project through evidence gates")
    subparsers = parser.add_subparsers(dest="command", required=True)

    init = subparsers.add_parser("init", help="lock one human-approved radar candidate")
