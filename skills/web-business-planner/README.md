...
研究 SERP 后产出不蚕食的页面矩阵和功能契约。它是 `web-business-pipeline` 套件的阶段入口，不复制中央状态机。
...
- “研究这个已锁定 SaaS 机会的 SERP 和替代方案，做 page-matrix.json”
- “给工具站建立一页一意图的页面矩阵，并先写功能契约”
...
```bash
ln -s /absolute/path/to/nemo-skills/skills/web-business-planner /absolute/path/to/project/.agents/skills/web-business-planner
...
```bash
npx skills add <owner/repository> --skill web-business-planner
```

## 工作方式

1. 运行中央 `status` 和 `validate`。初次模式只接受 `candidate_locked`；优化/扩展模式必须由 `$web-business-expander` 明确传入当前 `grow` 变更批次及模式。
2. 围绕锁定机会和目标客户做只读 SERP、替代方案与竞品研究，记录 query、地区/语言、检查时间、结果 URL、页面类型、搜索意图和未满足需求。竞品只用于信息架构判断，不复制文案、品牌或视觉。
