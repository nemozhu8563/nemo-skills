...
把跑通站点拆成框架、配置和内容三层模板。它是 `game-site-pipeline` 套件的阶段入口，不复制中央状态机。

## 适用边界

- 起点：`grow` 且第一个站点已有人工批准的有效数据决策。
- 终点：进入 `templated`。
- 不适用：把首个尚无数据的站点提前抽成框架；连同游戏内容、品牌、来源和统计 ID 一起复制；以模板化为理由自动批量建十个站。

## 你可以直接这样说

- “把这个 grow 游戏站做三层分离并抽成本地模板”
- “抽出游戏配置，让导航、SEO 和 sitemap 动态生成”
- “用第二个游戏做替换测试，确认 template_readiness 再进入 templated”
...
```bash
ln -s /absolute/path/to/nemo-skills/skills/game-site-templater /absolute/path/to/project/.agents/skills/game-site-templater
...
```bash
npx skills add <owner/repository> --skill game-site-templater
```
