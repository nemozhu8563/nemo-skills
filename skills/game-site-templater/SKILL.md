---
name: game-site-templater
description: "Extract a reusable local game-site template from a proven grow project by separating framework, per-game configuration, and independent content, removing game-specific branding, sources, telemetry IDs, domains, and secrets, then verifying a second-game substitution before marking templated. Use for 游戏站模板化、框架配置内容三层分离、抽站点模板、第二个游戏替换测试、动态导航 sitemap SEO、template_readiness；not for templating an unvalidated site, copying product-specific content, publishing a template, mass-cloning sites, or treating a working codebase as reusable proof."
---

# Game Site Templater

把第一个已验证站点中真正通用的基础设施提炼出来，同时显式排除游戏专属品牌、内容、证据、域名和统计身份。
...
- game-specific assets, claims, sources, domains and telemetry IDs are excluded
- navigation, routes, SEO and sitemap derive from config/content
- a second-game substitution test is reviewed before approval
...
2. 盘点站点并分三层：框架层（布局、组件、生成逻辑）、配置层（游戏名、主题色、导航、官方链接、SEO、locale）、内容层（各页事实、文案、翻译和来源）。先记录边界再改代码。
3. 把游戏名、主题色、官方链接、导航、SEO 标题/描述等硬编码集中到配置；页面、路由、导航、结构化数据和 sitemap 从配置/内容动态生成。只做当前模板所需的最小重构。
4. 从模板中移除或参数化产品专属品牌、文案、截图、兑换码、数值、source IDs、domain、GSC/GA property、广告 ID、环境配置和任何 secret。内容层不得作为示例默认发布。
5. 在隔离的本地目标中用第二个虚拟/无商标游戏配置做 substitution test：验证只改配置与内容即可切换，框架无需游戏专属改动；运行 build、links、assets、SEO、sitemap 和残留扫描。
...
- 框架/配置/内容三层边界清单
- 无产品身份和凭证的本地模板
- 第二游戏 substitution test 与残留扫描证据
