...
- 起点：初次建站为 `deploy_ready`；优化/扩展重发模式由 `$web-business-expander` 在 `grow` 且 QA 通过后调用。
- 终点：初次建站进入 `deployed`；优化/扩展模式保持 `grow` 并交给 telemetry。
...
1. 运行中央 `status`/`validate`，读取 launch report、目标 revision、canonical origin、回滚步骤和现有授权。初次模式只接受 `deploy_ready`；优化/扩展模式必须带 grow + 已通过 optimization/expansion QA 的上下文和完整变更批次。
...
6. 初次模式只有 deployment 为 verified、source revision 明确、匹配授权有效且公共 HTTP readback passed 时，才 `gate --target deployed` 并转移。优化/扩展模式运行 `validate --stage deployed`，保持 `grow` 并交给 `$web-business-telemetry`。
...
- deployed gate 或 optimization/expansion deployment validation 结果
