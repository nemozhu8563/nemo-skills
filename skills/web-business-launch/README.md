...
逐项授权执行上线动作并记录真实线上回读。它是 `web-business-pipeline` 套件的阶段入口，不复制中央状态机。
...
```bash
ln -s /absolute/path/to/nemo-skills/skills/web-business-launch /absolute/path/to/project/.agents/skills/web-business-launch
...
```bash
npx skills add <owner/repository> --skill web-business-launch
```
