...
为每页建立双来源和 claim 级证据包。它是 `game-site-pipeline` 套件的阶段入口，不复制中央状态机。

## 适用边界

- 起点：初次建站为 `planned`；扩页模式仅由 `$game-page-expander` 在 `grow` 中调用。
- 终点：初次建站进入 `researched`；扩页模式保持 `grow`。
- 不适用：复制竞品全文或专有数据；用 AI 摘要制造第二来源；在证据不足时补出兑换码、数字、日期或官方 URL。

## 你可以直接这样说

- “为页面矩阵建立 evidence-pack.json，每页至少双来源”
- “核验兑换码、数值和官方链接的 claim-level 证据”
...
```bash
ln -s /absolute/path/to/nemo-skills/skills/game-site-evidence /absolute/path/to/project/.agents/skills/game-site-evidence
...
```bash
npx skills add <owner/repository> --skill game-site-evidence
```
