...
按页面契约和证据实现页面及内容清单。它是 `web-business-pipeline` 套件的阶段入口，不复制中央状态机。
...
- “按页面契约实现这些功能页和落地页并生成 content-manifest.json”
...
```bash
ln -s /absolute/path/to/nemo-skills/skills/web-business-builder /absolute/path/to/project/.agents/skills/web-business-builder
...
```bash
npx skills add <owner/repository> --skill web-business-builder
```
