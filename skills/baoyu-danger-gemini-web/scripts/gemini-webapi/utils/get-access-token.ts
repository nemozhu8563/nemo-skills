...
function extract_init_token(html: string): string | null {
  const legacy = html.match(/\"SNlM0e\":\"(.*?)\"/);
  if (legacy?.[1]) return legacy[1];

  // Newer Gemini app pages expose the request token via WIZ_global_data.FdrFJe
  const current = html.match(/\"FdrFJe\":\"(.*?)\"/);
...
async function send_request(cookies: Record<string, string>, verbose: boolean): Promise<[string, Record<string, string>]> {
  ...
  const token = extract_init_token(text);
  if (!token) throw new Error('Missing init token in response');
