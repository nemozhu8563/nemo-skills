...
3. No package install step is required. This skill vendors its helper code under `scripts/vendor/`
4. Resolve `${BUN_X}` runtime: if `bun` installed → `bun`; if `npx` available → `npx -y bun`; else suggest installing bun
5. Replace all `{baseDir}` and `${BUN_X}` in this document with actual values
...
| `scripts/vendor/*` | Vendored helper code shipped with the skill, no extra install needed |
