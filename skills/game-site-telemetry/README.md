...
核验 GSC/GA、索引与观察窗口并安排复查。它是 `game-site-pipeline` 套件的阶段入口，不复制中央状态机。
...
```bash
ln -s /absolute/path/to/nemo-skills/skills/game-site-telemetry /absolute/path/to/project/.agents/skills/game-site-telemetry
...
```bash
npx skills add <owner/repository> --skill game-site-telemetry
```
