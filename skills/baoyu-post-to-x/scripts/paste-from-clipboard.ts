...
function pasteWindows(retries: number, delayMs: number, targetApp?: string): boolean {
  const ps = targetApp
    ? `
...
"@
      $proc = Get-Process | Where-Object { $_.MainWindowTitle -like "*${targetApp}*" } | Select-Object -First 1
      if ($proc) {
...
function paste(retries: number, delayMs: number, targetApp?: string): boolean {
  switch (process.platform) {
    ...
    case 'win32':
      return pasteWindows(retries, delayMs, targetApp);
    default:
