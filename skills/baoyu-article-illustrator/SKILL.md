...
### Step 1.5: Detect Image Directory

**Purpose**: Find the appropriate directory for storing generated images.
...
3. If configured, use that directory (relative to vault root)
4. Otherwise, use current logic: `illustrations/{topic-slug}/`

**Obsidian Config Detection**:
- Attachment folder path: `attachmentFolderPath` in `.obsidian/app.json`
- Special values: `"."` (vault root), `"/"` (vault root), or relative path
- Default: `"attachments"` if not configured

**Path Handling**:
- Calculate relative path from article to detected image directory
- For Obsidian, use wiki-link format
- Example: Article at `vault/notes/article.md`, images at `vault/assets/article-name/` → `![[assets/article-name/illustration-xxx.png]]`
...
2. **Determine which questions to ask**:
   ...
   | Language | Only if user explicitly specifies `--lang` or different language is detected |
...
3. **Present options** (use AskUserQuestion with all applicable questions):
   ...
   **Question 2 (Language)** - only if explicitly requested:
   - Chinese (default, for Chinese content)
   - English (for English content)
   - Other specified language

   **Language handling**:
   - Default: Use Chinese for all prompts and illustration text
   - If user explicitly specifies `--lang en` or similar: Use that language
   - If source language ≠ user language AND user explicitly requests: Ask which language to use for prompts
   - Otherwise: Inform user of default language (e.g., "将默认生成中文配图")
...
**Language Default**: Unless explicitly specified otherwise by user (e.g., `--lang en`), all prompts default to Chinese. All text content in illustrations (labels, titles, captions) will be in Chinese.
...
**IMPORTANT**: All text in the illustration must be displayed in [LANGUAGE] unless explicitly specified otherwise.
```
...
Insert generated images at corresponding positions using Obsidian wiki-link format:

```markdown
![[assets/{article-name}/illustration-[slug].png]]
```

**Path Calculation**:
- If using Obsidian-detected directory with `assets/${noteFileName}`: `![[assets/{article-name}/illustration-xxx.png]]`
- If using other Obsidian directory: `![[{detected-dir}/illustration-xxx.png]]`
- If using default `illustrations/{topic-slug}/`: `![[illustrations/{topic-slug}/illustration-xxx.png]]`
- Example results:
  - `![[assets/我用 Article Clip 搭了个素材库/illustration-concept-overview.png]]`
  - `![[images/illustration-workflow.png]]`

**Insertion Rules**:
- Insert image after corresponding paragraph
- Leave one blank line before and after image
- No alt text needed for Obsidian wiki-links (they use filename)
...
- **Language Default**: All prompts and illustration text default to Chinese unless user explicitly specifies `--lang en` or other language
- Prompts written in user's confirmed language preference (default: Chinese)
- Illustration text (labels, captions) should match article language (default: Chinese)
